...
const PosTab = () => {
    ...
    useEffect(() => {
        cargarDatos();
        const carritoGuardado = JSON.parse(localStorage.getItem('pos_carrito'));
    ...
    const finalizarVenta = async (metodoPago, avance) => {
        ...
        try {
            ...
            alert(`Error al procesar la venta:\n\n${errorMsg}\n\n${errorDetails ? 'Ver consola (F12) para más detalles' : ''}`);
        }
    };
    ...
    return (
        <div style={{ display: 'flex', width: '100%', height: 'calc(100vh - 120px)', backgroundColor: '#f1f5f9' }}>
